...
{
	int index1 = l, index2 = mid+1, curr = l;
	int* temp = (int*)Alloc((r+1)*sizeof(int));
	...
	//Copy the data from temp back into pData
	for (int i = l; i < r+1; i++)
	{
		pData[i] = temp[i];
		//printf("%d, ", pData[i]);
...
void mergeSort(int pData[], int l, int r)
{
	...
	//If the item has 1 element, return
	if (l >= r)
	{
		//printf("Exited\n");
		return;
